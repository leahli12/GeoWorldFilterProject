    ...
    public void go() {
        ...
        for (int row = 1; row < 700; row = row + 1) {
            for (int col = 0; col < 556; col++) {
                ...
                // How much of a color there is in a specific pixel
//                plane.teleport(col, 488 - row);
                if ((350/278.5)*col <= row) {
                    plane.setPixelColor(green, green, green);
                }
                if (col < 6){
                    plane.setPixelColor(row/3, row/3, row/3);
                }
                 if (row < 699 && row > 694) {
                    plane.setPixelColor(255-col/3, 255-col/3, 255-col/3);
                }
            }
        }
        plane.pausetime = 1;
        ...
        plane.isTrail = true;
        plane.setColor(100, 20, 20);
        plane.trailWidth = 5;
        plane.move(278.5);
        plane.setColor(146, 51, 49);
        plane.move(278.5);
        plane.startingAngle(90);
        plane.setColor(215, 162, 82);
        plane.move(350);
        plane.setColor(59, 91, 102);
        plane.move(350);
